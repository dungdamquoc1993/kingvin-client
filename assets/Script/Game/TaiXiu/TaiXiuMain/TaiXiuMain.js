...
cc.Class({
	...
	init: function(obj) {
		...
		this.onDiceAnimationFinish = function(event){
			...
			if (this.isNan) {
				...
				if (this.diemSo > 10) {
					this.efTai.play()
					this.efnodeTai.active = true;
				}else{
					this.efXiu.play()
					this.efnodeXiu.active = true;
				}
				if (this.diemSo%2) {
					this.efLe.play()
	...
	onLoad () {
		...
		this.onCuocClick = function(){
			...
			if (isNaN(bet) || bet < 1000) {
				...
				noticeComponent.text.string = "Tiền cược phải lớn hơn 1.000 " + (self.red ? "XU" : "Xu");
	...
	onSelectInput: function(event, select){
		...
		switch(select) {
			case 'right':
				this.input = this.inputRTxt;
			break;

			case 'left':
				this.input = this.inputLTxt;
			break;
		}
	},
	...
	setPhien:function(){
		var phien = cc.RedT.setting.taixiu.logs[0].phien;
		...
		if(void 0 !== data.finish){
			if (cc.RedT.setting.taixiu.getLogs) {
				...
				if(this.isNan){
					...
					this.diaNan.position       = cc.v2(0, 15);
	...
	efStop: function(){
		...
		this.efLe.stop();
		this.efChan.stop();
		this.efnodeXiu.active = false;
		this.efnodeTai.active = false;
	},
	moDia: function(){
		this.dataLogs();
		this.nodeKetQua.active = true;
	...
	playTime: function(){
		...
		this.timeInterval = setInterval(function() {
			if (cc.RedT.setting.taixiu.time_remain > 61) {
				...
				if (this.diaNan.x >= 180) {
					this.moDia();
					this.abc = true;
				}if (this.diaNan.x <= -200) {
					this.moDia();
					this.abc = true;
				}
				if (this.diaNan.y >= 190) {
					this.moDia();
					this.abc = true;
				}if (this.diaNan.y <= -145) {
					this.moDia();
					this.abc = true;
				}
				...
				if (cc.RedT.setting.taixiu.time_remain > 0) {
					...
					if (cc.RedT.setting.taixiu.time_remain <= 10){
						...
						if (cc.RedT.setting.taixiu.time_remain <= 5){
							this.animXoay.speed = 10;
						}
					}else{
						this.timeCuoc.node.color = cc.Color.WHITE
						this.animXoay.speed = 1;
					}
	...
	onDataChanLe: function(data){
		if (this.red) {
			void 0 !== data.red_chan && helper.numberTo(this.totalLeft, helper.getOnlyNumberInString(this.totalLeft.string), data.red_chan, 900, true);
			void 0 !== data.red_le && helper.numberTo(this.totalRight, helper.getOnlyNumberInString(this.totalRight.string), data.red_le, 900, true);
			void 0 !== data.red_me_chan && helper.numberTo(this.myLeft, helper.getOnlyNumberInString(this.myLeft.string), data.red_me_chan, 900, true);
			void 0 !== data.red_me_le && helper.numberTo(this.myRight, helper.getOnlyNumberInString(this.myRight.string), data.red_me_le, 900, true);
			void 0 !== data.red_player_chan && helper.numberTo(this.playerLeft, helper.getOnlyNumberInString(this.playerLeft.string), data.red_player_chan, 900, true);
			void 0 !== data.red_player_le && helper.numberTo(this.playerRight, helper.getOnlyNumberInString(this.playerRight.string), data.red_player_le, 900, true);
		}else{
	...
	onDataTaiXiu: function(data){
		...
		if (this.red) {
			void 0 !== data.red_tai && helper.numberTo(this.totalLeft, helper.getOnlyNumberInString(this.totalLeft.string), data.red_tai, 900, true);
			void 0 !== data.red_xiu && helper.numberTo(this.totalRight, helper.getOnlyNumberInString(this.totalRight.string), data.red_xiu, 900, true);
			void 0 !== data.red_me_tai && helper.numberTo(this.myLeft, helper.getOnlyNumberInString(this.myLeft.string), data.red_me_tai, 900, true);
			void 0 !== data.red_me_xiu && helper.numberTo(this.myRight, helper.getOnlyNumberInString(this.myRight.string), data.red_me_xiu, 900, true);
			void 0 !== data.red_player_tai && helper.numberTo(this.playerLeft, helper.getOnlyNumberInString(this.playerLeft.string), data.red_player_tai, 900, true);
			void 0 !== data.red_player_xiu && helper.numberTo(this.playerRight, helper.getOnlyNumberInString(this.playerRight.string), data.red_player_xiu, 900, true);
		}else{
	...
	dataLogs: function(){
		if (!!cc.RedT.setting.taixiu.logs.length) {
			...
			var c_tai = 0;
			var c_xiu = 0;
			var sliced = cc.RedT.setting.taixiu.logs.slice(0, 22);
			sliced.reverse();
			// Line paint
			var Paint = new Promise(function(resolve, reject){
				for (var i = 0; i < sliced.length; i++) {
					...
					if (void 0 !== data) {
						...
						line_dice1[i] = {x:i*39, y:dice1*37-37, dice: dice1};
						line_dice2[i] = {x:i*39, y:dice2*37-37, dice: dice2};
						line_dice3[i] = {x:i*39, y:dice3*37-37, dice: dice3};
						line_tong[i]  = {x:i*39, y:tong*11.5-27.7, tong: tong}
					}else{
