...
export default class Play extends cc.Component {
            ...
            switch (route) {
                ...
                case "OnLeavePlayer": {
                    ...
                    if (username == Configs.Login.UsernameFish) {
                        if (data["reason"] == 1) {
                            Lobby.instance.showNotice(true, "Bạn được mời ra khỏi phòng do không thao tác trong thời gian dài.");
                        }
                        this.back();
                    }
                ...
                ShootFishNetworkClient.getInstance().ping(() => {
                    ...
                    ShootFishNetworkClient.getInstance().request("play", {
                        ...
                        if (!res["ok"]) {
                            switch (res["err"]) {
                                case 4:
                                    Lobby.instance.showNotice(true, "Số dư không đủ vui lòng nạp thêm.");
                                ...
                                case 1:
                                    ...
                                    Lobby.instance.showNotice(true, "Lỗi " + res["err"] + ", vui lòng thử lại.");
                                    
                                    break;
                                default:
                                    Lobby.instance.showNotice(true, "Lỗi " + res["err"] + ", không xác định.");
    ...
    private getState(isFirst: boolean) {
        if (!isFirst) Lobby.instance.showLoading(true);
        this.isStateGeted = false;

        this.resetView();

        ShootFishNetworkClient.getInstance().request("state", null, (res) => {
            if (!isFirst) Lobby.instance.showLoading(false);
    ...
    private updateShoot(dt: number) {
        ...
        if (this.curShootInterval > 0) {
            ...
            if (Configs.Login.CoinFish < this.listBet[this.betIdx]) {
                //console.log("Số dư không đủ, vui lòng nạp thêm.");
                Lobby.instance.showNotice(true, "Số dư không đủ, vui lòng nạp thêm.");
    ...
    private back() {
        this.isStateGeted = false;
        this.stopAutoShoot();
        Lobby.instance.showLoading(true);
        ShootFishNetworkClient.getInstance().request("quit", null, () => {
            Lobby.instance.showLoading(false);
            this.resetView();
            this.show(false);
            this.lobby.getComponent(Lobby).show(true);
        }, this);
    ...
    public show(isShow: boolean, roomId: number = 0) {
        if (isShow) {
            if (Play.SERVER_CONFIG == null) {
                this.lobby.getComponent(Lobby).show(true);
                Lobby.instance.showNotice(true, "Bạn chưa đăng nhập.");
                return;
            }
