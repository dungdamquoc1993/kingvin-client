import Dialog2 from "./scripts/common/Dialog2";
...
export class TabCashIn {
    ...
    public submit() {
        ...
        if (coin <= 0) {
            Lobby.instance.showNotice(true, "Số tiền đã nhập không hợp lệ.");
            return;
        }
        ShootFishNetworkClient.getInstance().request("xxengCashin", { "ccash": coin }, (res) => {
            console.log(res['ok']);
            if (res['ok'] == false) {
                Lobby.instance.showNotice(true, "Thao tác thất bại, vui lòng thử lại sau.");
                return;
            }
            else{
                Configs.Login.CoinFish = res["newCash"];
                BroadcastReceiver.send(BroadcastReceiver.USER_UPDATE_COIN);
                Configs.Login.Coin = res["Cash"];
                cc.RedT.userData({data:{red:res["Cash"]}});
                Lobby.instance.showNotice(true, "Thao tác thành công.");
                this.reset();
            }
...
@ccclass
export default class PopupCoinTransfer extends Dialog2 {
