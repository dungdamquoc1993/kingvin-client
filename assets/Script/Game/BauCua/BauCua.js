...
cc.Class({
	...
	datCuoc: function(e, linhVat){
		//cc.log(linhVat);
		this.logcuoc.red[linhVat] += parseInt(this.cuoc);
	...
	updateMe: function(data){
		if (data.red.huou > 0) {
			this.linhVat[0].meCuocLog(data.red.huou);
		}
		if (data.red.bau > 0) {
			this.linhVat[1].meCuocLog(data.red.bau);
		}
		if (data.red.ga > 0) {
			this.linhVat[2].meCuocLog(data.red.ga);
		}
		if (data.red.ca > 0) {
			this.linhVat[3].meCuocLog(data.red.ca);
		}
		if (data.red.cua > 0) {
			this.linhVat[4].meCuocLog(data.red.cua);
		}
		if (data.red.tom > 0) {
			this.linhVat[5].meCuocLog(data.red.tom);
		}	
	},
	clickCuoc: function(){
		if (this.cuoc < 1000) {
			...
			this.updateMe(this.logcuoc);
			cc.RedT.send({g:{baucua:{cuoc:{cuoc:this.cuoc, red: this.red, linhVat: this.logcuoc}}}});
